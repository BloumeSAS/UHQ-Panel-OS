  ...
  async create(dto: CreatePoolDto) {
    ...
    const pool = await this.prisma.proxyPool.create({
      data: {
        ...
        antiVpnEnabled: dto.antiVpnEnabled ?? false,
        trafficMultiplier: dto.trafficMultiplier ?? 1,
    ...
    try {
      const pool = await this.prisma.proxyPool.update({
        ...
        data: {
          ...
          ...(dto.antiVpnEnabled !== undefined && { antiVpnEnabled: dto.antiVpnEnabled }),
          ...(dto.trafficMultiplier !== undefined && { trafficMultiplier: dto.trafficMultiplier }),
